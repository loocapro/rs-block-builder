...
use crate::service::metrics::PayloadBuilderServiceMetrics;
use crate::traits::{PayloadJob, PayloadJobGenerator};
...
impl PayloadBuilderHandle {
    ...
    pub async fn resolve(&self, id: PayloadId) -> Option<Result<Arc<Build>, PayloadBuilderError>> {
    ...
    pub async fn best_payload(
        &self,
        id: PayloadId,
    ) -> Option<Result<Arc<Build>, PayloadBuilderError>> {
        let (tx, rx) = oneshot::channel();
        self.to_service
    ...
    pub fn build_new_payload(
        ...
        let _ = self
            .to_service
            .send(PayloadServiceCommand::BuildNewPayload(Box::new(config), tx));
        rx
    }
    ...
    pub async fn new_payload(&self, config: BuildConfig) -> Result<PayloadId, PayloadBuilderError> {
        self.build_new_payload(config).await?
    }
}
    ...
    Gen: PayloadJobGenerator,
{
    ...
    fn best_payload(&self, id: PayloadId) -> Option<Result<Arc<Build>, PayloadBuilderError>> {
        self.payload_jobs
            .iter()
    ...
    fn resolve(&mut self, id: PayloadId) -> Option<PayloadFuture> {
        ...
        if keep_alive == KeepPayloadJobAlive::No {
            let _ = self.payload_jobs.remove(job);
        }
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        loop {
            ...
            for idx in (0..this.payload_jobs.len()).rev() {
                ...
                match job.poll_unpin(cx) {
                    Poll::Ready(Ok(_)) => {
                        ...
                        debug!(target: "payload::service", ?id, "Payload job finished");
                    }
                    Poll::Ready(Err(err)) => {
                        error!(target: "payload::service", ?id, ?err, "Payload builder job failed; resolving payload");
            ...
            while let Poll::Ready(Some(cmd)) = this.command_rx.poll_next_unpin(cx) {
                match cmd {
                    PayloadServiceCommand::BuildNewPayload(config, tx) => {
                        let id = config.attributes.id;
                        let parent = config.attributes.parent;
                        let mut res = Ok(id);

                        if this.contains_payload(id) {
                            debug!(target: "payload::service", ?config, ?parent, "Payload job already in progress, ignoring.");
                        } else {
                            match this.generator.new_payload_job((*config).clone()) {
                                Ok(job) => {
                                    info!(target: "payload::service", ?config, "New payload job created");
                                    this.metrics.inc_initiated_jobs();
                                    new_job = true;
                                    this.payload_jobs.push((job, id));
                                }
                                Err(err) => {
                                    warn!(target: "payload::service",?config, ?err, "Failed to create payload builder job");
                                    this.metrics.inc_failed_jobs();
                                    res = Err(err);
                                }
                            }
...
type PayloadFuture =
    Pin<Box<dyn Future<Output = Result<Arc<Build>, PayloadBuilderError>> + Send + Sync>>;
...
pub(crate) enum PayloadServiceCommand {
    ...
    BestPayload(
        PayloadId,
        oneshot::Sender<Option<Result<Arc<Build>, PayloadBuilderError>>>,
    ),
...
impl fmt::Debug for PayloadServiceCommand {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            PayloadServiceCommand::BuildNewPayload(f0, f1) => f
            ...
            PayloadServiceCommand::PayloadAttributes(f0, f1) => f
