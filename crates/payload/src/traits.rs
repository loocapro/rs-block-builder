...
use builder_primitives::{build::Build, payload::BuildConfig};

use reth_payload_builder::{
    error::PayloadBuilderError, EthPayloadBuilderAttributes, KeepPayloadJobAlive,
};
...
pub trait PayloadJob: Future<Output = Result<(), PayloadBuilderError>> + Send + Sync {
    ...
    type ResolvePayloadFuture: Future<Output = Result<Arc<Build>, PayloadBuilderError>>
        + Send
        + Sync
        + 'static;
...
pub trait PayloadJobGenerator: Send + Sync {
    ...
    /// returned directly.
    fn new_payload_job(&self, build_config: BuildConfig) -> Result<Self::Job, PayloadBuilderError>;
}
